...
$TempDir = Join-Path $env:TEMP ("fq-share-uninstall-" + [System.Guid]::NewGuid().ToString("N"))
...
try {
  ...
  & $HostBinary uninstall --browser $Browser
  if ($LASTEXITCODE -ne 0) {
    throw "Native host uninstall command failed with exit code $LASTEXITCODE"
  }
} finally {
